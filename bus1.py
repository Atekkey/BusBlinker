...
def makeRegMatrix(fetch_weather):
    ...
    if temp != None:
        draw_string(arrCanv, temp+"^", off + 10*3, 0, color=9)
    ...
    tempPos = temp != None and int(temp) > 0
    if tempPos:
        dowMap = {"MON": "MO", "TUE": "TU", "WED": "WE", "THU": "TH", "FRI": "FR", "SAT": "SA", "SUN": "SU"}
    ...
    draw_string(arrCanv, date, off + 10*4, 1, color=4) # 12/05 WAS 1
    draw_string(arrCanv, hour_min, off + 10*5, 3, spacing = 1, color=5) # 01:04 WAS 5
        ...
        for y in range(2, array.shape[0]):
            for x in range(array.shape[1]):
                if array[y, x] == 1:
                    ...
                    canvas.SetPixel(x, y, 98, 174+25, 179) # LB2 
                elif array[y, x] == 6:
                    ...
                    a = -40
                    canvas.SetPixel(x, y, 255 + a, 95 + a, 5) # uiuc OR
                elif array[y, x] == 9:
                    a = 70
